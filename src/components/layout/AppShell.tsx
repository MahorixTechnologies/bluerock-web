    ...
    <Link
      ...
      className={`group relative flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition-all duration-200 ${
        active
          ...
          : "text-white/80 hover:bg-[var(--sidebar-hover)]/70 hover:text-white"
      }`}
    >
      {active ? (
        ...
        {items.map((item) => (
          <div key={item.href} className="relative">
            ...
            {badge && item.label === badge.label ? (
              <span className="absolute right-3 top-1/2 flex -translate-y-1/2 items-center justify-center rounded-full bg-[var(--danger)] px-2 py-0.5 text-[10px] font-bold text-white">
                {badge.value}
              </span>
            ) : null}
          </div>
...
function SidebarContents({
  ...
  profile: { name: string; email: string; role: UserRole } | null | undefined;
      ...
      <div className="border-t border-white/8 p-4">
        {status === "signedIn" && profile ? (
          ...
          <div className="rounded-2xl bg-white/6 p-4">
            ...
            <Link
              href="/login"
              className="mt-4 inline-flex w-full items-center justify-center rounded-xl bg-[var(--primary)] px-4 py-2.5 text-sm font-bold text-white shadow-[0_8px_20px_rgba(30,91,255,0.35)] transition hover:bg-[var(--primary-600)]"
            >
              Open Login
            </Link>
          </div>
...
export function AppShell({
  ...
  return (
    <main className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[272px] flex-col overflow-hidden bg-[var(--sidebar)] shadow-[8px_0_30px_rgba(10,42,140,0.25)] lg:flex">
        <SidebarContents
          navLabel={navLabel}
          mainNav={mainNav}
          status={status}
      ...
      {mobileNavOpen ? (
        <div className="fixed inset-0 z-40 lg:hidden" role="dialog" aria-modal="true">
          ...
          />
          <div
            className="relative flex h-full w-[280px] max-w-[85vw] flex-col overflow-hidden bg-[var(--sidebar)] shadow-2xl"
            onClick={() => setMobileNavOpen(false)}
          >
            <SidebarContents
              navLabel={navLabel}
      ...
      <div className="lg:pl-[272px]">
        <div className="mx-auto flex min-h-screen max-w-[1600px] flex-col gap-6 p-4 lg:p-6">
          <header className="sticky top-6 z-20 flex items-center justify-between gap-4 rounded-3xl border border-[var(--border)] bg-white/80 px-5 py-4 backdrop-blur-md shadow-[var(--shadow-soft)]">
            <div className="flex items-center gap-4">
              <button
                ...
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[var(--sidebar)]/8 text-[var(--sidebar)] transition hover:bg-[var(--sidebar)]/12 lg:hidden"
              >
                ☰
              </button>
              <div>
                <h1 className="text-[20px] font-black tracking-tight text-[var(--text)]">
                  {heading || "Dashboard"}
                </h1>
                {subheading ? (
                  <p className="mt-0.5 text-sm text-[var(--muted)]">{subheading}</p>
                ) : status === "signedIn" && profile ? (
            ...
            <div className="flex items-center gap-2 sm:gap-3">
                ...
                {status === "signedIn" && profile ? (
                  <>
                    ...
                    <button
                      type="button"
                      aria-label="Add"
                      className="flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--border)] bg-white text-[var(--muted)] transition hover:bg-[var(--panel-soft)]"
                    >
                      +
                    </button>
                  </>
              ...
              <button
                ...
                className="relative flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--border)] bg-white text-[var(--muted)] transition hover:bg-[var(--panel-soft)]"
              >
                ⏰
              ...
              {status === "signedIn" && profile ? (
                ...
                >
                  <span className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-[var(--primary)] to-[var(--sidebar)] text-[11px] font-black text-white">
